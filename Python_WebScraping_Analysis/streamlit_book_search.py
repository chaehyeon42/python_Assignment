...
def filter_books_by_publisher(df, publisher_name):
    ...
    columns_to_show = df.columns.drop(['image','description','link']) #출판사필터링 목록에서 'image','description','link' 제거
...
if search_button and client_id and client_secret:
    with st.spinner('검색 중...'):
        ...
        # API 호출 결과가 search_results에 저장되어 있다면 
...
with tab1:
    if not st.session_state.books_df.empty:
        ...
        #render_books_df 변수 생성하여 보고싶은 컬럼만 화면에 뿌려주도록 수정
